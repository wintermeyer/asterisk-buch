...
`T` oder `tT` aktiviert werden. Wie das genau geht,
wird in xref:applications-dial.adoc[Abschnitt C.38, „`Dial()`“] beschrieben. Wenn Sie sowohl
dem Anrufer wie auch dem Angerufenen das Transferieren von Gesprächen
erlauben wollen, müssen Sie die Optionen `tT` benutzen.
