...
Mit der Applikation `Playback()` (siehe xref:applications-playback.adoc[Abschnitt C.117, „`Playback()`“]) können Soundfiles nur abgespielt
werden. Jegliche Eingabe während des Abspielens wird nicht beachtet.
`Background()` (siehe xref:applications-background.adoc[Abschnitt C.13, „`Background()`“]) hingegen spielt die Datei ab und
...
Informationen zu `TIMEOUT` finden Sie bei xref:functions-timeout.adoc[Abschnitt D.101, „`TIMEOUT()`“].
====
