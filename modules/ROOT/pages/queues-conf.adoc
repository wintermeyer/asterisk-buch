...
beispielsweise eine Warteschlange im Abschnitt `[support]` an.
...
`ringall`:: Alle verfügbaren Interfaces gleichzeitig anklingeln, bis einer antwortet. (Default)
...
`rrordered`:: Wie `rrmemory`, hält aber zusätzlich die Reihenfolge aus der Konfigurationsdatei ein.
`linear`:: Klingelt in der Reihenfolge der `member`-Einträge. Bei dynamischen Members in der Reihenfolge des Einfügens.
`wrandom`:: Zufällig, aber mit Berücksichtigung der Penalty der Agenten als Gewichtung.

[NOTE]
====
Die alte Strategie `roundrobin` ist seit Asterisk 12 nicht mehr
verfügbar. Wählen Sie als Ersatz `rrmemory` (Round-Robin mit
Gedächtnis), `rrordered` (dasselbe, aber in Config-Reihenfolge) oder
`linear` (strikt nach `member`-Einträgen).
====
...
Weitere Informationen zu den Mitschnitte-Applikationen (`Monitor()`,
`MixMonitor()`) finden Sie unter https://docs.asterisk.org.
...
Details zu `Monitor()` und `MixMonitor()` finden Sie unter
https://docs.asterisk.org.

[[queues.conf-joinempty]]
== `joinempty`

Legt fest, ob ein Anrufer in eine Warteschleife eingereiht wird, wenn
dort keine Agenten verfügbar sind.

Moderner Asterisk erwartet als Wert eine kommagetrennte Liste von
Faktoren, die einen Member *unverfügbar* machen (z. B.
`paused,invalid,unavailable`). Trifft einer der Faktoren zu, gilt
der Member als nicht verfügbar. Gibt es keinen verfügbaren Member
mehr, wird der Anrufer nicht aufgenommen — die `Queue()`-Applikation
endet sofort, und der Dialplan läuft hinter ihr weiter.

Verfügbare Faktoren (Auszug): `paused`, `penalty`, `invalid`,
`inuse`, `ringing`, `wrapup`, `unavailable`, `unknown`.

Aus Kompatibilitätsgründen gibt es daneben die klassischen
Schlüsselwörter:

`yes`:: entspricht *leerer* Liste — jeder kommt rein.
`no`:: entspricht `penalty,paused,invalid` — der Default-Fall.
`strict`:: `penalty,paused,invalid,unavailable`.
`loose`:: `penalty,invalid`.

Achtung: Statisch in der `queues.conf` definierte Mitglieder (siehe
xref:queues-conf.adoc#queues.conf-member[`member`]) gelten immer als
erreichbar, auch wenn sie in Wirklichkeit nicht an ihrem Apparat
sind.
...
Legt fest, wann Anrufer aus einer laufenden Warteschlange *entfernt*
werden. Die Werte sind dieselben wie bei `joinempty` (modern:
kommagetrennte Liste von Faktoren; alt: `yes` / `no` / `strict` /
`loose`). Die Liste wird regelmäßig geprüft; sobald kein Member
mehr verfügbar ist, verlässt der Anrufer die Queue und der Dialplan
läuft hinter der `Queue()`-Applikation weiter.
...
– also z. B. `member => PJSIP/1001` – anzulegen
