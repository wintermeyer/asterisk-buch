[[warteschleifen-extensions.conf]]
= `extensions.conf`

Im Dialplan (xref:kapitel-programmieren.adoc[Kapitel 6, _Programmieren im Dialplan_]) können sich
...
`Queue()` (xref:applications-queue.adoc[Abschnitt C.122, „`Queue()`“]) in eine
...
`AgentCallbackLogin()` hingegen hat den Vorteil, dass
...
`doc/queues-with-callback-members.txt` (in AEL, lässt
...
Agenten, `AddQueueMember()` zum dynamischen Hinzufügen zur
Warteschlange und `Dial()` zum Anrufen der Agenten.

Sollte AgentCallbackLogin() tatsächlich ersatzlos wegfallen, gehen
