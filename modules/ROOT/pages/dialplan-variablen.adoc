...
arbeiten können. Alte Legacy-Namen wie `$\{CALLERIDNUM}` (ohne
Klammern) gibt es in modernen Asterisk-Versionen nicht mehr —
benutzen Sie die Funktion `$\{CALLERID(num)}`.
====
...
sie in der Variable `$\{VORWAHL}`.footnote:[Leider macht diese Variante in Deutschland wenig Sinn, da es unterschiedlich lange Vorwahlen gibt. Mehr Informationen zur deutschen Vorwahl finden Sie unter https://de.wikipedia.org/wiki/Telefonvorwahl_(Deutschland).]
