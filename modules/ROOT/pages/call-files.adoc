...
* Asterisk öffnet eine Verbindung zum Telefon `SIP/2000`. Wenn das Telefon besetzt sein oder den Anruf nicht annehmen sollte, dann versucht Asterisk das Gleiche noch zweimal (siehe _`MaxRetries`_).
...
`Channel: <channel>`:: Der zu benutzende Channel. Dabei gilt die gleiche Syntax wie beim `Dial()`-Befehl (siehe Abschnitt C.38, „`Dial()`“).
