...
im Context `interne-benutzer` klingelt.footnote:[Zur Beschreibung der Applikation `Pickup()` siehe auch: xref:applications-pickup.adoc[Abschnitt C.115, „`Pickup()`“\]]footnote:[Die Applikation `Pickup()` darf nicht verwechselt werden mit der Bristuff-Applikation `PickUp()` bzw. `DPickup()`, `PickUp2()` oder anderen Applikationen ähnlichen Namens, die in den diversen Internet-Foren usw. als Patches herumgeistern, auch wenn sie ungefähr das Gleiche bewirken.]
...
(xref:blf-telefon-konfiguration-snom.adoc[Abschnitt 23.3, „Konfiguration der Telefone für BLF“]) bei
...
`Pickup()`-Applikation (xref:applications-pickup.adoc[Abschnitt C.115, „`Pickup()`“]) an, mit der man flexibler ist.
Beispiel:
