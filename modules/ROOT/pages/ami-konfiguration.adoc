...
:description: manager.conf in Asterisk konfigurieren: User, Rechte, ACLs.

AMI ist das klassische, seit Jahren bestehende TCP-Protokoll, mit dem
externe Programme Asterisk fernsteuern und Ereignisse mitlesen. Für
*neue* Anwendungen, die Call-Control machen sollen (Gespräche anlegen,
umleiten, Audio abspielen, DTMF verarbeiten), empfehlen wir heute
xref:ari.adoc[ARI]. AMI ist dennoch nach wie vor sinnvoll:
für Monitoring-Skripte, für das Reagieren auf Events mit einfachen
Tools (`expect`, `nc`, kleinen Python-Skripten) und für Altbestand.
...
_`Befehlsname`_+ (siehe xref:ami-action-referenz.adoc[Anhang F, _AMI-Befehle_]):
...
[NOTE]
====
`expect` ist ein erweiterter Tcl-Interpreter, mit dem man
automatisiert interaktive Shell-Programme ansteuern
kann.footnote:[https://core.tcl-lang.org/expect/, https://en.wikipedia.org/wiki/Expect]
====
...
So könnten wir das Beispiel aus xref:ami-konfiguration.adoc#manager-interface-beispiel-expect[„Beispiel: Anzahl der Mailbox-Nachrichten mit Expect
