...
Im Dialplan (xref:dialplan-programmieren.adoc[Kapitel 6, _Programmieren im Dialplan_]) können sich
...
`Queue()` (Abschnitt C.122, „`Queue()`“) in eine
...
exten => 20,n,Set(MUSICCLASS()=default)  ; Musikklasse "default" setzen
exten => 20,n,Queue(support)             ; in Warteschlange "support" einreihen
; setzt QUEUESTATUS =
...
Wenn wir diese Extension anrufen,
werden wir, da keine Agenten angemeldet sind, die Ansage hören, dass zur
Zeit[xref:queues-conf.adoc#ftn.footnote-zurzeit[85]] niemand unseren Anruf
entgegennehmen kann. (Wenn Sie das verschleiern wollen, setzen Sie in der
`queues.conf` die Parameter `joinempty=yes`
und `leaveempty=no`.)
