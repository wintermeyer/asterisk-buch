...
; deaktivieren
exten => 44,1,Answer()
exten => 44,n,NoOp(${DB_DELETE(CF/${CALLERID(num)})})
...
exten => _X.,n,Dial(Local/${DB(CF/${EXTEN})}@from-intern)
----
...
; deaktivieren
exten => 55,1,Answer()
exten => 55,n,NoOp(${DB_DELETE(CF/anlage)})
...
exten => _X.,n,Dial(Local/${DB(CF/anlage)}@from-intern)
----
