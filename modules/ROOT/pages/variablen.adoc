...
(_Real_) arbeiten wollen, können Sie dies mit
einem AGI-Skript realisieren (siehe xref:kapitel-agi.adoc[Kapitel 20, _Asterisk Gateway Interface (AGI)_]).
====
...
in einer Extension zu setzen.footnote:[siehe auch xref:applications-set.adoc[Abschnitt C.149, „`Set()`“\]]
...
in der Datei `doc/README.variables` (Asterisk
1.2) und `doc/channelvariables.txt` (Asterisk
1.4). In dieser Liste werden nur solche Variablen aufgelistet, die
nicht als _deprecated_ eingestuft sind. So finden
Sie z. B. ein `${CALLERIDNUM}` nicht in dieser Liste,
weil dies deprecated ist. (Sie sollten stattdessen die Funktion
`${CALLERID(num)}` benutzen.)
====
